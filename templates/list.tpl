<html>
	...
	<body onload="displayError()">
		<div id="thread">
			...
			<div>1 名前:{if $info.email}<a href="mailto:{$info.email}">{/if}{if $info.name}<span id=name>{$info.name}</span>{else}<span id=name>ななしさん</span>{/if}{if $info.email}</a>{/if}  投稿日時：{$info.created}</div>
				<div>{$info.description}</div>
		
			</br>
			{foreach from=$comment key=k item=v}
				<div class="comment">
					<div>{$k + 2} 名前:{if $v.email}<a href="mailto:{$v.email}">{/if}{if $v.name}<span id=name>{$v.name}</span>{else}<span id=name>ななしさん</span>{/if}{if $v.email}</a>{/if}  投稿日時：{$v.created}</div>
